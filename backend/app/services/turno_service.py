...
def update_turno(db: Session, turno_id: int, turno_update: TurnoUpdate):
    ...
    if not db_turno: return None

    check_dia_abierto(db, db_turno.fecha_hora.date())
    ...
    if "fecha_hora" in update_data and update_data["fecha_hora"]:
        ...
        check_dia_abierto(db, nueva_fecha.date())
    ...
    if "servicios_ids" in update_data:
        ...
        for s_id in update_data["servicios_ids"]:
            srv = db.query(Servicio).filter(Servicio.id == s_id).first()
            if srv:
...
def get_horarios_semanales(db: Session, empleado_id: int, fecha_inicio: date):
    resultado = {}
    horarios_laborales = db.query(HorarioEmpleado).filter(HorarioEmpleado.empleado_id == empleado_id).all()
    ...
    for i in range(7):
        ...
        while actual + timedelta(minutes=30) <= fin:
            ...
            slots.append({
                ...
                "cliente":    f"{turno_existente.cliente.nombre} {turno_existente.cliente.apellido}" if turno_existente else None,
